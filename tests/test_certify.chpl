...
proc main() throws {
  ...
  const res = runSieve(nf, 0.332813, maxDepth=20, useUnits=true, unitExponentRange=1, maxProblems=50000);
  ...
  if !res.cleared && res.numRemaining > 0 {
    ...
    for i in 1..numToScan {
      ...
      const exact = exactMinimalNormAt("x^3-3*x-1", box.center, nf.degree, searchRange=3);
