...
record LezowskiKernel {
  ...
  // Computes the strict upper bound of the norm function over the projected mixed intervals/disks
  proc calculateBoxMaxNorm(ref centers: [] complex(128), ref radii: [] real(64)): real(64) {
    var maxNorm: real(64) = 1.0;
    
    // 1. Upper bound for Real Embeddings (Interval maximum)
    for i in 1..this.r1 {
      var c_val = abs(centers[i].re);
      var r_val = radii[i];
      maxNorm *= (c_val + r_val); // Maximum absolute distance from 0 in this interval
    }
    
    // 2. Upper bound for Complex Embeddings (Disk maximum: (|z_0| + R)^2)
    for j in 1..this.r2 {
      var idx = this.r1 + j;
      var z0 = centers[idx];
      ...
      var max_z = z0_mag + R;
      maxNorm *= (max_z * max_z); // Absolute square of the outermost point of the disk
    }
    
    return maxNorm;
  }

  // Sieve Routine: Verifies if an entire continuous box is covered below the target bound
  proc testBoxCovering(ref box: Box, targetBound: real(64) = 1.0): bool {
    var centers: [1..this.num_embeddings] complex(128);
    var radii: [1..this.num_embeddings] real(64);
    
    // Project the abstract algebraic box into embedding spaces
    this.computeBoxProjections(box, centers, radii);
    
    // Check if the base box region is completely covered by 0
    if this.calculateBoxMaxNorm(centers, radii) < targetBound then
      return true;
      
    // Apply Fundamental Unit transformation checks across the whole interval/disk region
    if this.numUnits > 0 {
      for u in 1..this.numUnits {
        ...
        for r in 1..this.num_embeddings {
          // Complex translation rotates and scales the center point
          shiftedCenters[r] = centers[r] * this.unitsMatrix[r, u];
          
          // Scaling the radius of the disk/interval by the magnitude of the unit embedding
          var u_val = this.unitsMatrix[r, u];
          var u_mag = sqrt(u_val.re * u_val.re + u_val.im * u_val.im);
          shiftedRadii[r] = radii[r] * u_mag;
        }
        
        // Evaluate the maximum possible norm of the shifted region
        if this.calculateBoxMaxNorm(shiftedCenters, shiftedRadii) < targetBound then
          return true; // The entire box area passes under the sieve via this unit transformation
      }
    }
    
    return false; // Box cannot be safely cleared; must be broken down further
  }
}
