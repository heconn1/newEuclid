...
record LezowskiKernel {
  ...
  // Sieve Routine with Integer Translation Step
  proc testBoxCovering(ref inputBox: Box, targetBound: real(64) = 1.0): bool {
    ...
    // FIXED: Strict guard to verify we actually have unit vectors to process
    if this.numUnits > 0 {
...
proc runAMRTreeSieve(ref kernel: LezowskiKernel, targetBound: real(64) = 1.0, maxDepth: int = 12) {
  ...
  for depth in 1..maxDepth {
    ...
    // FIXED: Explicitly shadow and bind cDom layout to ensure clean thread allocation scopes
...
proc main() {
  ...
  runAMRTreeSieve(kernel, targetBound = 1.0, maxDepth = 10);
}
